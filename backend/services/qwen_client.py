...
1. 识别页面中的视觉元素（重点关注用户查询的特定内容）：
   - **图表(Figure)**：彩色图片/图像 + 文字标题的组合格式
     * 包括：柱状图、折线图、散点图、饼图、流程图、示意图等
     * 特征：主体为彩色图像内容，下方通常有"Figure X. 标题"格式的文字说明
   - **表格(Table)**：文字数据表格 + 标题的组合格式
     * 包括：数据表、对比表、统计表等规整的表格结构
     * 特征：主体为文字和数字组成的表格，有明确的行列结构和边框
   - 其他元素（公式、纯文字等，非主要目标）
...
【关键定位规则 - 极其重要】
**对于图表(Figure)的边界定位，必须严格遵循以下规则：**

1. **顶边定位（图表上边界）**：
   - 顶边应该是彩色图片/图表内容的最上边
   - 不要遗漏图表上方的任何彩色内容
   - 如果图表上方有标题，且标题紧贴图表，则从标题开始
   - 绝对不能少截取彩色图片的任何部分

2. **底边定位（图表下边界）**：
   - 底边应该是Figure标题文字完结的位置
   - 包含完整的"Figure X. 标题描述"文字
   - 确保Figure编号和完整标题都被包含在内
   - 不要截断Figure标题的任何文字

3. **左右边界定位**：
   - 左边界：图表内容的最左边，包括坐标轴标签
   - 右边界：图表内容的最右边，包括图例（如果有）
   - 确保图表的所有组成部分都被完整包含

4. **完整性检查**：
   - 确保截取区域包含：彩色图表主体 + 坐标轴 + 图例 + Figure标题
   - 避免包含过多的空白区域，但不能遗漏任何图表组件
   - 特别注意不要因为追求紧凑而截断重要内容

**对于表格(Table)的边界定位，必须严格遵循以下规则：**

1. **顶边定位（表格上边界）**：
   - 如果表格上方有标题（"Table X. 标题描述"），则从标题开始
   - 如果没有上方标题，则从表格的第一行（通常是表头）开始
   - 确保包含完整的表格标题文字

2. **底边定位（表格下边界）**：
   - 底边应该是表格最后一行的结束位置
   - 如果表格下方有说明文字或注释，也应包含在内
   - 确保表格的所有数据行都被完整包含

3. **左右边界定位**：
   - 左边界：表格最左列的开始位置
   - 右边界：表格最右列的结束位置
   - 确保所有列的内容都被完整包含，包括列标题

4. **完整性检查**：
   - 确保截取区域包含：表格标题 + 表头 + 所有数据行 + 表格边框
   - 特别注意表格的边框线，确保完整的表格结构
   - 避免截断任何单元格内容

【精确定位要求】
- 仔细观察图表和表格的实际边界，包括图表本身、标题、坐标轴、图例等完整内容
- 确保边界框紧密贴合图表/表格的实际范围，但优先保证完整性而非紧凑性
- 特别注意图表/表格的左上角坐标要准确，避免偏移
- 宽度和高度要覆盖图表/表格的完整内容，包括标题和图例/表头
- **重点强调**：
  - 对于Figure：顶边必须包含完整的彩色图片，底边必须包含完整的Figure标题文字
  - 对于Table：顶边必须包含完整的Table标题，底边必须包含表格最后一行和任何注释

【置信度评分标准 - 重要】
请根据以下标准为每个检测到的Figure和Table分配置信度分数：

**重要区分说明：**
- **Figure（图表）**：主体必须是彩色图像/图片内容，如柱状图、折线图、流程图、示意图等视觉化图形
- **Table（表格）**：主体必须是文字和数字组成的规整表格结构，有明确的行列划分和边框
...
**高置信度 (0.9-1.0)：标准学术Table格式**
- 规整的表格结构 + 明确的标题（如"Table 1. xxx"）
- 表格有清晰的表头、数据行和边框
- 标题与表格内容高度相关，描述清晰
- 整体布局规范，符合学术论文标准

**中等置信度 (0.7-0.9)：部分符合Table格式**
- 有表格结构但标题不够明确或位置不标准
- 表格结构基本完整但边框可能不清晰
- 表格内容较简单但结构可识别

**低置信度 (0.5-0.7)：可能的Table元素**
- 有表格样式的数据排列但缺少明确标题
- 表格结构不够规整或部分缺失
- 表格边框不清晰或数据排列混乱
...
**极低置信度 (0.3-0.5)：疑似Figure/Table**
- 纯文字段落（即使包含"Figure"或"Table"字样）
- 页面布局元素（页眉、页脚等）
- 不完整或截断的图表/表格

**不是Figure/Table (0.0-0.3)：明确排除**
- 纯文字内容
- 页面装饰元素
- 明显不是图表或表格的内容

【关键要求】
- 论文页面中会有标题（如3.1）等会被误认为是Figure/Table，需要特别注意，这不是用户想要的
- 论文中会有页面的文字部分提及"Figure num"或"Table num"，这是论文对于Figure/Table的讲解，不是用户想要的
- 如果用户查询特定Figure（如"Figure 1"），则ONLY返回该Figure，不要返回其他Figure或Table
- 如果用户查询特定Table（如"Table 1"），则ONLY返回该Table，不要返回其他Table或Figure
- 如果页面中没有用户查询的目标Figure/Table，则返回空的figures数组
- 确保返回的Figure/Table与用户查询完全匹配
- 优先识别有明确标号的Figure和Table
- 定位精度要求：坐标误差应控制在±2%以内
- **严格按照置信度评分标准，优先识别：**
  - Figure：彩色图片+文字标题的标准Figure格式
  - Table：规整表格结构+明确标题的标准Table格式
...
{{
  ...
  "figures": [
    {{
      ...
      "boundary_quality": "边界质量评估(excellent/good/fair/poor)",
      "completeness_check": {{
        "has_color_content": "是否包含彩色内容(true/false,主要针对Figure)",
        "has_title": "是否包含标题(true/false,Figure/Table标题)",
        "title_position": "标题位置(above/below/none)",
        "element_type": "元素类型(figure/table)"
      }}
    }}
  ]
}}
...
- **边界质量评估标准**：
  - excellent: 边界完美贴合，包含所有必要元素
  - good: 边界基本准确，可能有轻微偏差
  - fair: 边界大致正确，但可能遗漏部分内容
  - poor: 边界不准确，需要人工调整
- **完整性检查要求**：
  - 必须明确标注是否包含彩色内容（主要针对Figure）
  - 必须明确标注是否包含标题文字（Figure/Table标题）
  - 必须标注标题的相对位置
  - 必须明确标注元素类型（figure或table）
- 如果无法确定精确位置，请给出最佳估计并降低置信度
- 如果页面中没有匹配的Figure/Table，请返回空的figures数组
- 只返回与用户查询匹配的Figure/Table，避免截取无关内容
- **关键要求**：确保每个返回的Figure/Table都经过完整性验证，优先保证截取完整性而非精确性
"""
